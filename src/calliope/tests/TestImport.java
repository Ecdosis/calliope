...
public class TestImport extends Test
{
    ...
    +"(\"DOC_ID\");\n\t\thidden.value =docid;\n\t\t\tvar password="
    +"prompt(\"Password\",\"\");\n\t\tvar demo = document.getEleme"
    +"ntById(\"demo\");\n\t\tif ( demo != null )\n\t\t{\n\t\t\tdem"
    +"o.value = password;\n\t\t\treturn true;\n\t\t}\n\t\telse\n\t"
    +"\t\treturn false;\n\t}\n\telse\n\t\treturn false;\n}\nfuncti"
    +"on check_files()\n{\n\tvar repo= document.getElementById(\"r"
    +"epository\");\n\tvar child = repo.firstChild;\n\tvar numChil"
    +"dren = 0;\n\twhile ( child != null )\n\t{\n\t\tif ( child.no"
    +"deName==\"INPUT\" )\n\t\t\tnumChildren++;\n\t\tchild = child"
    +".nextSibling;\n\t}\n\tif ( numChildren == 0 )\n\t{\n\t\taler"
    +"t( \"specify at least one file for upload\" );\n\t\treturn f"
    +"alse;\n\t}\n\telse\n\t\treturn true;\n}\nfunction fverify( i"
    +"tem )\n{ \n\tvar name = \"\";\n\tif ( item !=null )\n\t{\n\t"
    ...
    +"ument.getElementById(\"repository\");\n\tvar child = repo.fi"
    +"rstChild;\n\twhile ( child != null )\n\t{\n\t\tif ( child.no"
    +"deName==\"INPUT\"&& child.value==value )\n\t\t{\n\t\t\tchild"
    +".parentNode.removeChild(child);\n\t\t\tbreak;\n\t\t}\n\t\tch"
    +"ild = child.nextSibling;\n\t}\n\tvar table = document.getEle"
    ...
    +"d!=null&&ggchild.nodeName==\"SPAN\" )\n\t\t\t\t\t{ \n\t\t\t\t"
    +"\t\tif ( ggchild.textContent!=null&&ggchild.textContent==va"
    +"lue )\n\t\t\t\t\t\t{\n\t\t\t\t\t\t\tfinished = true;\n\t\t\t"
    +"\t\t\t\tchild.parentNode.removeChild(child);\n\t\t\t\t\t\t\t"
    +"break;\n\t\t\t\t\t\t}\n\t\t\t\t\t}\n\t\t\t\t}\n\t\t\t\tgchil"
    +"d = gchild.nextSibling;\n\t\t\t}\n\t\t}\n\t\tchild = child.n"
    +"extSibling;\n\t}\n}\nfunction alreadySelected( path, listing"
    +" )\n{\n\tvar row = listing.firstChild;\n\twhile ( row != nul"
    +"l )\n\t{\n\t\tif ( row.firstChild.textContent==path )\n\t\t\t"
    +"return true;\n\t\trow = row.nextSibling;\n\t}\n\treturn fal"
    +"se;\n}\nfunction clearFileInput( input1 )\n{\n\tvar parent ="
    +" input1.parentNode;\n\tvar input2 = document.createElement('"
    +"input');\n\tinput2.setAttribute( \"type\", \"file\" );\n\tin"
    +"put2.setAttribute( \"name\", \"uploadedfile[]\" );\n\tinput2"
    +".setAttribute( \"onchange\", \"doaddfile()\" );\n\tinput1.se"
    +"tAttribute( \"class\", \"invisible\" );\n\tinput1.removeAttr"
    +"ibute(\"id\");\n\tinput2.setAttribute(\"id\",\"input1\");\n\t"
    +"parent.removeChild( input1 );\n\tparent.appendChild( input2"
    +" );\n}\nfunction doaddfile()\n{\n\tvar input1 = document.get"
    +"ElementById(\"input1\");\n\tvar repo = document.getElementBy"
    +"Id(\"repository\");\n\tvar listing = document.getElementById"
    +"(\"listing\");\n\tif ( !alreadySelected(input1.value,listing"
    ...
    +"'\"+input1.value.replace(/\\\\/g,\"\\\\\\\\\")+\"')\");\n\t\t"
    +"button.setAttribute(\"class\",\"remove\");\n\t\tbutton.setA"
    +"ttribute(\"value\",\"remove\");\n\t\tvar cell2 = document.cr"
    +"eateElement(\"td\");\n\t\trow.appendChild( cell2 );\n\t\tcel"
    +"l2.appendChild( button );\n\t\tif ( listing.firstChild == nu"
    +"ll )\n\t\t\tlisting.appendChild( row );\n\t\telse \n\t\t lis"
    +"ting.insertBefore( row, listing.firstChild );\n\t}\n\telse\n"
    +"\t{\n\t\tclearFileInput( input1 );\n\t\talert(\"You have alr"
    +"eady chosen that file!\");\n\t}\n}\nfunction update_group()\n"
    +"{\n\tvar corform = document.getElementById(\"CORFORM\");\n\t"
    +"var groupSpan = document.getElementById(\"GROUP\");\n\tif ("
    +" groupSpan != undefined )\n\t{\n\t\tvar path = corform.optio"
    +"ns[corform.selectedIndex].value;\n\t\tvar parts = path.split"
    +"(\"/\");\n\t\tvar group = \"\";\n\t\tfor ( var i=0;i<parts.l"
    +"ength-1;i++ )\n\t\t{\n\t\t\tif ( group.length > 0 )\n\t\t\t\t"
    +"group += \"-\";\n\t\t\tgroup += parts[i];\n\t\t}\n\t\tif ( "
    +"group.length > 0 )\n\t\t\tgroup += \": \";\n\t\tgroupSpan.te"
    +"xtContent = group;\n\t}\n}";
