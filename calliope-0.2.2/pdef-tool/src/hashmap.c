...
static int hashmap_rehash( hashmap *map )
{
	...
	if ( new_buckets == NULL )
	{
		fprintf(stderr,"hashmap: failed to resize hash table\n");
        return 0;
	}
...
hashmap_iterator *hashmap_iterator_create( hashmap *map )
{
    ...
    if ( iter != NULL )
    {
        ...
        {
            fprintf(stderr,"hashmap_iterator: couldn't allocate key array\n");
        }
    }
    else
        fprintf(stderr,"hashmap_iterator: failed to allocate object\n");
    return iter;
}
/**
